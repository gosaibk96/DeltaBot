...
RENDER_APP_URL = os.environ.get('RENDER_EXTERNAL_URL', '')  # Auto-detects Render URL

@app.route('/')
def home():
    return "Supertrend Bot Active & Awake!"
    ...
    while True:
        ...
        if RENDER_APP_URL:
            try:
                ...
                print("⏰ Keep-Alive Ping Sent to Render!", flush=True)
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get('success') and 'result' in data and len(data['result']) > 0:
                ...
                return df
        print("API Response Issue:", res.text, flush=True)
        return None
    except Exception as e:
...
def place_order(side, reduce_only=False):
    try:
        ...
        print(f"Order Placement Exception: {e}", flush=True)
        return {}
...
def strategy_loop():
    ...
    print("🚀 BOT ALIVE & RUNNING...", flush=True)
            ...
            if df is not None and len(df) > ST_PERIOD + 2:
                ...
                # CLOSED CANDLE ENTRY
                    ...
                    if closed_price > st_val and last_signal_direction == -1:
                        print("🟢 CANDLE CLOSED ABOVE: Buying...", flush=True)
                        ...
                        print("🔴 CANDLE CLOSED BELOW: Selling...", flush=True)
