...
def execute_breakout_trade(side, trigger_price):
    ...
    if returned_trail is None or returned_tp is None:
        log(f"BRACKET NOT CONFIRMED on order {order_id}. Emergency closing to avoid naked position.")
    ...
    if fill_price:
        log(f"ENTRY {side.upper()} FILLED @ {fill_price}")
...
def on_ws_close(ws, close_status_code, close_msg):
    log(f"WebSocket closed. Reconnecting...")
...
def candle_watcher_loop():
    ...
    log(f"Candle watcher started. Resolution={CANDLE_RESOLUTION}")
        ...
        try:
            ...
            if pending_start is not None:
                ...
                if candle:
                    ...
                    log("Candle fetch failed. Skipping this candle.")
                    with state_lock:
                        shared_state["pending_candle_start"] = None
                        shared_state["pending_deadline"] = None
    ...
    while True:
        try:
            ...
            if size != 0 and not locally_open:
                ...
                log(f"Position synced -> size={size}, entry_price={entry_price}")

            elif size == 0 and locally_open:
                with state_lock:
                    shared_state["position_open"] = False
                    shared_state["position_side"] = None
                log("Position CLOSED (flat). Ready for next breakout.")
