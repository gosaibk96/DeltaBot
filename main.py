...
# =====================================================================
# ⚙️ EASY CUSTOMIZATION PARAMETERS (YAHAN SE VALUES CHANGE KAREIN)
# =====================================================================
...
# 1. Fetch Candles from Delta API
def fetch_candles():
    try:
        resolution_map = {"1m": "1", "5m": "5", "15m": "15", "1h": "60"}
        res_val = resolution_map.get(TIMEFRAME, "1")
        
        end_time = int(time.time())
        start_time = end_time - (100 * int(res_val) * 60)
        
        path = f"/v2/chart/history?symbol={SYMBOL}&resolution={res_val}&start={start_time}&end={end_time}"
...
# 4. Pure Supertrend Execution Loop
def strategy_loop():
    time.sleep(5)
    current_position = None  # Tracks live position: None, "BUY", or "SELL"
    ...
    while True:
        try:
            ...
            if df is not None and len(df) > ST_PERIOD:
                ...
                # SELL SIGNAL: Price < Supertrend & Position not already SELL
